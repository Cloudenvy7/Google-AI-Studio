...
It produces a meeting note that reads as if Andrew wrote it, then stages it for the dossier.
...
- **Plain language, no jargon inflation.** Low fog index.
- **Name things specifically** — tools (Bolt.new, NotebookLM, Perplexity, Claude), events,
  programs, people. Never abstract a named thing into a category.
- **Follow-ups split cleanly by owner** (client vs. advisor), each tactical and concrete.
- Use the same bullet style throughout; bold lead-ins are acceptable for themed recommendations.
...
5. On approval, hand off:
   ...
   - Emit an **Event** (`EVT-`) + a **Commit** to `Project_Narrative_Log` with a Framework tag
     (this is the dual-layer requirement) — delegate to the `historian-narrative-commit` skill.

## Honest limits

- True Google Docs **tabs** cannot be created via API. If the dossier needs a new tab and Drive
  for Desktop sync produces a section rather than a tab, say so and fall back to the browser path
  or a clearly-separated dated section.
